...
const MANUAL_INSTALL_STEPS: Record<InstallPlatform, string> = {
  ...
  desktop: "Look for the install icon in your browser's address bar, or open the browser menu and choose \"Install Budget Planner\".",
};
...
export function TabBar({ active }: { active: Route }) {
  ...
  async function onBrandClick() {
    if (installed) {
      setInstallNote("Budget Planner is already installed on this device.");
      return;
    }
    if (canPrompt) {
  ...
  return (
    <>
      ...
      <BottomSheet open={!!installNote} title="Install Budget Planner" onClose={() => setInstallNote("")}>
